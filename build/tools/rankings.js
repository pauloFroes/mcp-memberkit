...
export function registerRankingTools(server) {
    server.registerTool("list_rankings", {
        title: "List Rankings",
        description: "List member rankings (leaderboard). Returns position, score, and user data for each entry. Optionally filter by classroom ID. Supports pagination.",
        inputSchema: {
            page: z
                .string()
                .optional()
        ...
        try {
            ...
            return toolError(`Failed to list rankings: ${error.message}`);
        }
    });
    server.registerTool("create_score", {
        title: "Create Score",
        description: "Award points to a member. Requires a reason, point value, member email, and course ID.",
        inputSchema: {
        ...
        try {
            ...
            return toolError(`Failed to create score: ${error.message}`);
        }
    });
    server.registerTool("delete_score", {
        title: "Delete Score",
        description: "Remove a score entry by matching user email, reason, and course ID. This action is irreversible.",
        inputSchema: {
            user_email: z
                .string()
        ...
        try {
            ...
            return toolError(`Failed to delete score: ${error.message}`);
        }
    });
}

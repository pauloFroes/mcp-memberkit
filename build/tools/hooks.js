...
export function registerHookTools(server) {
    server.registerTool("list_hooks", {
        title: "List Webhooks",
        description: "List all configured webhooks. Returns webhook URL, status, monitored events, and timestamps. Supports pagination.",
        inputSchema: {
            page: z
                .string()
                .optional()
        ...
        try {
            ...
            return toolError(`Failed to list hooks: ${error.message}`);
        }
    });
    server.registerTool("create_hook", {
        title: "Create Webhook",
        description: "Create a new webhook to receive event notifications. Requires a URL and an array of event names to monitor.",
        inputSchema: {
        ...
        try {
            ...
            return toolError(`Failed to create hook: ${error.message}`);
        }
    });
    server.registerTool("update_hook", {
        title: "Update Webhook",
        description: "Update an existing webhook. Can change URL, status, and monitored events.",
        inputSchema: {
        ...
        try {
            ...
            return toolError(`Failed to update hook: ${error.message}`);
        }
    });
    server.registerTool("delete_hook", {
        ...
        try {
            ...
            return toolError(`Failed to delete hook: ${error.message}`);
        }
    });
}

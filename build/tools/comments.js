...
export function registerCommentTools(server) {
    server.registerTool("list_comments", {
        title: "List Comments",
        description: "List all comments in the membership area. Filter by status (unread, pending, approved, rejected), sort by field and order. Returns comment content, status, lesson info, and user data.",
        inputSchema: {
            page: z
                .string()
                .optional()
    ...
    server.registerTool("get_comment", {
        title: "Get Comment",
        description: "Get details of a specific comment by ID. Returns comment content, status, parent comment ID, classroom ID, lesson info (with course), and user data.",
        inputSchema: {
        ...
        try {
            ...
            return toolError(`Failed to get comment: ${error.message}`);
        }
    });
    server.registerTool("create_comment", {
        title: "Create Comment",
        description: "Create a new comment on a lesson. Requires content text, user ID, lesson ID, classroom ID, and status. Optionally reply to another comment via parent_id.",
        inputSchema: {
        ...
        try {
            ...
            return toolError(`Failed to create comment: ${error.message}`);
        }
    });
    server.registerTool("approve_comment", {
        ...
        try {
            ...
            return toolError(`Failed to approve comment: ${error.message}`);
        }
    });
    server.registerTool("reject_comment", {
        ...
        try {
            ...
            return toolError(`Failed to reject comment: ${error.message}`);
        }
    });
    server.registerTool("delete_comment", {
        title: "Delete Comment",
        description: "Permanently delete a comment by ID. This action is irreversible.",
        inputSchema: {
